...
import { IChartFilter, IFeedProperties } from '../models/ChartInterfaces';
...
export class DataSource {
    ...
    fetch(feed:IFeedProperties, attributes:string[] = [], type : ChartTypes, dataMetric : DataMetrics, historyLength: number, filters: IChartFilter) {
        ...
        return DataServiceProvider.fetchEntityDataByFeed(feed).then((res) => {
            const args = { inputData: res, attributes, type, inputType: DataIOTypes.Entity, outputType: DataIOTypes.Entity, dataMetric, historyLength, filters };
