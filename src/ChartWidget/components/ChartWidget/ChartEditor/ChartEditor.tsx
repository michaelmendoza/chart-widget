...
import { ChartModes, ChartTypes } from '../../../models/ChartEnums';
...
const ChartEditor = () => {
    ...
    useEffect(() => {
        ...
        if (showCreator) {
            const fetch = async () => {
                const availableCharts : any = await DataServiceProvider.fetchAvailableFeeds();
                dispatch({ type: ActionTypes.UPDATE_CHART_EDITOR, 
                    editor: {...editor, 
                    availableFeeds: availableCharts, 
                    feed: availableCharts[0], 
                    feedId: availableCharts[0].id, 
                    feedName: availableCharts[0].name}});
            };
            fetch();
        }
        ...
        if (showEditor) {
            ...
            { type: ActionTypes.UPDATE_CHART_EDITOR, 
              editor: {...editor, 
                ...
                name: chart.name, 
                feedName: chart.feedName, 
                attributes: chart.attributes, 
                metric: chart.dataMetric, 
    ...
    const handleSave = () => {
        ...
        const chartItem = new ChartItem(editor.name, 
            editor.chartType, 
            editor.feed,
            editor.feedId,
            editor.feedName, 
        ...
        if (state.chartConfig.mode === ChartModes.ShowChartCreator) {
            ...
            dispatch({type: ActionTypes.UPDATE_CHART, id, updatedChart: chartItem});
            dispatch({type: ActionTypes.UPDATE_CHART_MODE, mode: ChartModes.ShowCharts});
        }
    };
